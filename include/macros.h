...
/* Lowest address a real pointer can have.
 *
 * Several places sanity-check a pointer by comparing it against the base of
 * Dreamcast RAM (0x8C010000) -- if it is below that it cannot be a real
 * pointer, so it is null or garbage. Xbox RAM starts at 0x00000000, so that
 * literal rejects EVERY valid Xbox pointer; the texture cache in particular
 * was sending every single texture down its "bad address" path. The Xbox
 * equivalent is the title image base: nothing legitimate lives below it. */
#if defined(TARGET_XBOX)
#define GAME_RAM_MIN_ADDR 0x00010000u
#else
#define GAME_RAM_MIN_ADDR 0x8c010000u
#endif
