...
void* soundAlloc(struct SoundAllocPool* pool, u32 size) {
    ...
    if (start + alignedSize <= pool->start + pool->size) {
        ...
        /* Upstream leaves this silent, so an exhausted pool returns NULL into
           an unchecked memcpy and faults somewhere unrelated. Say so instead. */
        printf("AUDIO soundAlloc FAILED want=%u start=%p cur=%p size=%u free=%d\n",
               (unsigned) size, (void *) pool->start, (void *) pool->cur,
               (unsigned) pool->size,
               (int) ((pool->start + pool->size) - pool->cur));
#endif
