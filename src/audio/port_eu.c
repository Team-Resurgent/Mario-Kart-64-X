...
/* WHITE-dump diagnostic: how many audio commands the audio thread has ever
 * processed. Zero means the game->audio command ring never delivers. */
int xbox_snd_cmds_processed = 0;
unsigned char xbox_snd_ops[16];                    /* first ops seen, for the dump */
...
void func_800CBCB0(u32 arg0) {
    ...
    for (;;) {
        ...
        if (xbox_snd_cmds_processed < 16) xbox_snd_ops[xbox_snd_cmds_processed] = cmd->u.s.op;
        xbox_snd_cmds_processed++;
        //printf("cmd is %08x\n\top is %02x\n", cmd, cmd->u.s.op);
