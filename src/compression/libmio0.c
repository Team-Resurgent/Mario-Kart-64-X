...
#include <string.h>
#include <stdlib.h>
/* The Xbox target is x86-windows-gnu, so _WIN32 is defined, but it is a
 * freestanding picolibc environment -- pulling mingw's <io.h> here drags in a
 * conflicting off_t and creat. This branch is for host builds of the standalone
 * tool, which the console build is not. */
