...
/* Xbox is a freestanding picolibc target that still defines __MINGW32__ via
 * the x86-windows-gnu triple; <direct.h> would drag in mingw's off_t and
 * varargs, which conflict. The console build has no filesystem-creation need
 * here anyway. */
#if (defined(_MSC_VER) || defined(__MINGW32__)) && !defined(TARGET_XBOX)
  #include <direct.h>
