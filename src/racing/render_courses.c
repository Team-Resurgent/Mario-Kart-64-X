...
s32 func_80290C20(Camera* camera) {
     ...
     * course's under-water/ice "masking floor" while the white-explosion bug
     * was being chased. That was the wrong culprit -- the explosion was
     * armature limb matrices overflowing to NaN (see
     * convert_to_fixed_point_matrix_animation) -- and suppressing the floors
     * has its own cost: the award ceremony drives up to the castle on Royal
     * Raceway, whose floor (packed_dl_B030) is gated here, and it flickered
     * without it. Back to the original behaviour; only Sherbet's 27A8 floor
     * stays deliberately unemitted, as the DC port shipped it (see the note
     * in the COURSE_SHERBET_LAND case). */
    s32 result = 0;
    if (camera->collision.unk34 == 0) {
        result = 1;
    } else if ((camera->collision.unk30 == 1) && (camera->collision.surfaceDistance[0] < 3.0f)) {
        result = 1;
    } else if ((camera->collision.unk32 == 1) && (camera->collision.surfaceDistance[1] < 3.0f)) {
        result = 1;
    }

#if defined(TARGET_XBOX) && MK64X_DEBUG_TOOLS
    /* Ceremony diagnostic. The award scene flies a cinematic camera whose
     * collision fields may not be maintained frame to frame; if this gate
     * flips, the masking floor (packed_dl_B030 on Royal Raceway, which the
     * ceremony uses) appears and disappears -- exactly what a flickering
     * floor looks like. Report every FLIP, not every frame, plus the inputs,
     * so one ceremony run says whether the gate is the cause. */
    if (gGamestate == ENDING) {
        extern int printf(const char*, ...);
        static s32 sPrev = -1;
        static int sPrints = 0;
        if (result != sPrev && sPrints < 24) {
            sPrints++;
            sPrev = result;
            printf("CEREMFLOOR gate=%d unk34=%d unk30=%d unk32=%d sd0=%d sd1=%d /1000\n",
                   (int) result, (int) camera->collision.unk34,
                   (int) camera->collision.unk30, (int) camera->collision.unk32,
                   (int) (camera->collision.surfaceDistance[0] * 1000.0f),
                   (int) (camera->collision.surfaceDistance[1] * 1000.0f));
        }
    }
#endif
    return result;
        ...
        case COURSE_KOOPA_BEACH:
            ...
            switch (pathCounter) {
                ...
                case 37:
                    ...
                    // Cleared of suspicion: the white explosion persisted with
                    // this skipped -- the real culprit was the func_80290C20
                    // masking-floor gate (see the probe itself).
