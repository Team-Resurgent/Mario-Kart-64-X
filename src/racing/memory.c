...
// The Dreamcast version walks the stack by reading r15/pr directly and
// bounding the scan to SH-4 RAM addresses. Neither the registers nor the
// address range mean anything on x86; RXDK's own crash handler covers this
// ground on Xbox, so this is inert rather than reimplemented.
__attribute__((noinline)) void stacktrace(void) { }
#else
...
#if defined(TARGET_XBOX)
    /* The Dreamcast can tell a real pointer from a segmented address by range
       alone: its RAM lives at 0x8C000000, far above the segment space, which is
       what the check above does. Xbox RAM starts at 0x00000000, so a genuine
       heap pointer such as 0x017A5080 is indistinguishable BY RANGE from
       segment 1 offset 0x7A5080 -- and was being translated as one, which is
       how the audio loader ended up reading a sequence count of 31234 out of a
       buffer whose header plainly says 29.
       Distinguish by meaning instead of by range: an address is only a
       segmented address if that segment has actually been registered by
       set_segment_base_addr, and the game never uses segments 0 or 1 for data
       (see the bad-segment check below). Anything else is a real pointer.
       Residual risk, stated plainly: once the heap grows past 0x02000000 a real
       pointer can carry a segment number that IS registered, and would be
       mistranslated. Fixing that properly means allocating the segment-backed
       buffers somewhere segment numbers cannot reach. */
    if (segment < 2 || gSegmentTable[segment] == 0) {
        return (void *) uip_addr;
    }

    /* Second discriminator, for the residual ambiguity noted above.
     *
     * A genuine segmented address carries an offset INTO that segment's data,
     * and no segment here is more than a few hundred KB (common_data is the
     * largest at 184KB). A real heap pointer reinterpreted as a segmented
     * address yields an offset of many megabytes -- 0x02ABCDEF would be
     * "segment 2, offset 11MB". Treat an implausible offset as proof that this
     * is a real pointer, not a segmented one.
     *
     * This leaves a 4MB window per segment where the two are still
     * indistinguishable. The only complete fix is to place real allocations
     * where segment numbers cannot reach, which the Dreamcast gets for free by
     * having its RAM at 0x8C000000. */
    if ((uip_addr & 0x00FFFFFFu) >= 0x00400000u) {
        static int warned = 0;
        if (warned < 8) {
            warned++;
            printf("SEG passthrough: %08x looks like a real pointer, not seg %u\n",
                   (unsigned) uip_addr, (unsigned) segment);
        }
        return (void *) uip_addr;
    }
#endif
