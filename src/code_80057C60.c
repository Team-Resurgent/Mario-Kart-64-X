...
void func_8005A070(void) {
    ...
    /* Watermark: report frames that would have overflowed the ORIGINAL
     * 800-entry mtxHud pool (the water-course explosion suspect). */
    {
        extern int printf(const char*, ...);
        static u16 sMaxHud = 0;
        static int sPr = 0;
        if (gMatrixHudCount > sMaxHud) {
            sMaxHud = gMatrixHudCount;
            if (sMaxHud > 700 && sPr < 10) {
                sPr++;
                printf("MTXHI hud peak=%d\n", (int) sMaxHud);
            }
        }
    }
#endif
...
void func_8005C980(void) {
    s32 var_v0;
    // no doubt about it, used uninitialized
    s32 sp0 = 0;
    s32 temp_v1;
