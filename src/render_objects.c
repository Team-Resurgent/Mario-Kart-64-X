...
void shadow_for_a_hedgehog(s32 objectIndex, f32 arg1) {
    ...
    if ((is_obj_flag_status_active(objectIndex, 0x00000020) != 0) &&
        ...
        set_transform_matrix(sp30, object->unk_01C, D_80183E50, 0U, arg1);
        {
            ...
            convert_to_fixed_point_matrix(&gGfxPool->mtxHud[slot], sp30);
            gSPMatrix(gDisplayListHead++, VIRTUAL_TO_PHYSICAL(&gGfxPool->mtxHud[slot]),
...
void render_object_bat(s32 cameraId) {
    ...
    /* activeTLUT/activeTexture are assigned by update_bat's state machine --
     * init_texture_object only fills tlutList/textureList -- and Time Trials
     * SKIPS update_bat (see the course update switch). Rendering anyway loaded
     * a NULL palette: harmless on N64 (address 0 is real RDRAM) and on the DC
     * (MMU off), an access violation on Xbox. The particle loops below draw
     * with this same texture, so there is nothing to draw without it. */
    if (gObjectList[objectIndex].activeTLUT == NULL || gObjectList[objectIndex].activeTexture == NULL) {
        return;
    }
...
void func_80055FA0(s32 objectIndex, UNUSED s32 arg1) {
    ...
    if (object->state >= 2) {
        ...
        {
            s32 slot = mtx_hud_alloc();
            convert_to_fixed_point_matrix(&gGfxPool->mtxHud[slot], someMatrix1);
            gSPMatrix(gDisplayListHead++, VIRTUAL_TO_PHYSICAL(&gGfxPool->mtxHud[slot]),
...
void func_80056FCC(s32 bombIndex) {
    ...
    {
        s32 slot = mtx_hud_alloc();
        convert_to_fixed_point_matrix(&gGfxPool->mtxHud[slot], mat);
        gSPMatrix(gDisplayListHead++, VIRTUAL_TO_PHYSICAL(&gGfxPool->mtxHud[slot]),
