...
/* Enlarged from 800 (and mtxObject from 128): object-dense sections of Koopa
 * Beach / Frappe Snowland / Sherbet Land exhausted the pools. mtxHud has
 * uncapped allocators that then overflow INTO mtxObject -- and because the
 * Xbox renderer defers the display-list walk to end-of-frame, matrices are
 * read by pointer at flush time, so an overflow rewrites matrices that
 * earlier course-section draws still reference: course chunks render with
 * garbage transforms (the "screen explodes white" bug). The allocators are
 * also hard-clamped now; see rsp_set_matrix_transformation. */
#define MTX_HUD_POOL_SIZE 2048
...
/* Clamped mtxHud slot allocator (math_util_2.c). Returns the next free slot,
 * or the last slot (without advancing, with a throttled MTXOV print) when the
 * pool is exhausted -- every gSPMatrix must keep pointing at valid pool
 * memory because the deferred DL walk dereferences these at end of frame. */
s32 mtx_hud_alloc(void);
