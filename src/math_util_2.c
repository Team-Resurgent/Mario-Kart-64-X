...
s32 mtx_hud_alloc(void) {
    if (gMatrixHudCount < MTX_HUD_POOL_SIZE) {
        return gMatrixHudCount++;
    }
    {
        ...
        if (sOv < 8) {
            sOv++;
            printf("MTXOV hud pool full (%d)\n", (int) MTX_HUD_POOL_SIZE);
        }
    }
    /* Reuse the last slot rather than overflow into mtxObject: an overflow
     * write rewrites matrices the deferred DL walk still dereferences for
     * earlier course-section draws -- geometry then renders with garbage
     * transforms (the water-course "screen explodes" bug). Reuse mangles at
     * most the one draw that owned this slot. */
    return MTX_HUD_POOL_SIZE - 1;
}
...
void rsp_set_matrix_transformation(Vec3f translate, Vec3su orientation, f32 scale) {
    ...
    convert_to_fixed_point_matrix(&gGfxPool->mtxHud[slot], matrix);
