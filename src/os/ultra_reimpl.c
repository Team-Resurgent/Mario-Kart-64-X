...
/* Everything below is the DREAMCAST save implementation: EEPROM (the game's
 * records and settings) and the Controller Pak (ghost data), both backed by a
 * VMU through KOS's maple + fs_* APIs and wrapped in VMU package headers.
 *
 * None of that exists on Xbox, and maple_enum_type() there reports no memory
 * card by design, so every one of these would fail and the game would conclude
 * it has nowhere to save. The Xbox versions live in
 * Platform/xbox/xbox_savegame.c, writing into a dashboard-visible save
 * container instead. */
...
#endif /* !TARGET_XBOX -- Dreamcast VMU save implementation */
