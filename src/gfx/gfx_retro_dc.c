...
#include "xbox_debug.h"   /* MK64X_DEBUG_TOOLS: the on-hardware dump switch */
#else
...
static struct RSP {
	...
	/* 32 deep (was 11): MK64's armature critters (seagulls/penguins/ghosts)
	 * nest 6 G_MTX_PUSHes per body; at the old cap pushes were silently
	 * skipped while their matching pops still ran, draining the stack to 0
	 * mid-frame -- and stack[size-1] then dereferenced stack[-1], garbage
	 * struct memory: the NaN/giant-white-triangle "explosion" near those
	 * critters' map areas. See gfx_sp_matrix/gfx_sp_pop_matrix. */
	float modelview_matrix_stack[32][4][4] __attribute__((aligned(32)));
...
static struct RDP {
	...
	struct {
		...
		uint8_t siz;
		uint8_t cms, cmt;
		uint8_t palette;             // CI4: selects one of 16 sub-palettes (tlut base = palette<<4)
...
#if defined(TARGET_XBOX)
// No store queue on x86. The Xbox backend batches geometry and hands it to
// D3D at end_frame, so the OP fast path is just the ordinary submit.
static inline void pvr_submit_op_inline(const dc_fast_t *tris, size_t n) {
    pvr_submit_op(tris, n);
}
#else
...
// The SH-4 build pins shz_dot8f's arguments to fr8-fr11 so the fipr
// instruction can read them in place. x86 has no such constraint.
#if defined(TARGET_XBOX)
#define SHZ_DOTREG(name) float name
#else
#define SHZ_DOTREG(name) register float name asm (#name)
#endif
...
void gfx_texture_cache_invalidate(void* orig_addr) {
	...
	uintptr_t last_node = (uintptr_t)&gfx_texture_cache.pool[gfx_texture_cache.pool_pos];
...
static  __attribute__((noinline)) uint8_t gfx_texture_cache_lookup(int tile, int dslot, struct TextureHashmapNode** n, const uint8_t* orig_addr,
	...
	uint64_t newkey = pack_key((uint32_t)(uintptr_t)segaddr,uls,ult,fmt,siz);
	...
	struct TextureHashmapNode** node = &gfx_texture_cache.hashmap[hash];
	if ((uintptr_t)rdp.loaded_texture[dslot].addr < GAME_RAM_MIN_ADDR) {
	...
	*node = &gfx_texture_cache.pool[gfx_texture_cache.pool_pos++];
	...
	(*node)->key = pack_key((uint32_t)(uintptr_t)segaddr,uls,ult,fmt,siz);
	(*node)->pad2 = dimkey;
...
/* CI4 was an empty stub (the dispatcher's commented-out abort): shell domes
 * and other 4-bit palettized art uploaded whatever stale pixels sat in
 * rgba16_buf -- the mottled garbage shells. High nibble is the LEFT pixel;
 * the tile's palette field selects one of 16 sub-palettes. */
static __attribute__((noinline)) void import_texture_ci4(int tile) {
	uint32_t width = rdp.texture_tile.line_size_bytes << 1;
	uint32_t height = rdp.loaded_texture[tile].size_bytes / rdp.texture_tile.line_size_bytes;
	const uint8_t* src = rdp.loaded_texture[tile].addr;
	const uint16_t* pal = &tlut[(uint32_t) rdp.texture_tile.palette << 4];
...
static __attribute__((noinline)) void import_texture_ci8(int tile) {
	...
	/* WHITE-dump: shells re-import every frame (their render invalidates the
	 * cache), so one press catches a live shell import: source pixels, the
	 * palette in force, and what they convert to. */
	{
		extern int printf(const char*, ...);
		static uint32_t last_dump_frame = 0;
		if (xbox_gfx_log_once && last_dump_frame != (uint32_t) (uintptr_t) rdp.loaded_texture[tile].addr) {
			last_dump_frame = (uint32_t) (uintptr_t) rdp.loaded_texture[tile].addr;
			const uint8_t* px = rdp.loaded_texture[tile].addr;
			/* Sample the MIDDLE of the texture (row 16 for 32-wide): edge
			 * rows are index 0 and told us nothing. For the green shell
			 * frame, ROM says px[512..519] ends ...00 04 05 01 and the
			 * palette entries for 1/4/5 are 0307/0AC7/0B07 -> converted
			 * 1555 = 8183/84D7/85C3-ish. Any other values name the culprit:
			 * wrong px = frames clobbered; wrong tlut = palette wrong. */
			printf("CI8 addr=%p line=%u size=%u px[mid]=%02X %02X %02X %02X %02X %02X %02X %02X "
			       "tlut[1,4,5,px]=%04X %04X %04X %04X\n",
			       (void*) px, (unsigned) width, (unsigned) rdp.loaded_texture[tile].size_bytes,
			       px[512], px[513], px[514], px[515], px[516], px[517], px[518], px[519],
			       tlut[1], tlut[4], tlut[5], tlut[px[517]]);
		}
	}
#endif
...
#if defined(TARGET_XBOX) && MK64X_DEBUG_TOOLS
	/* Boot diagnostic paired with the backend's TEXUP texel dump: name the
	 * importer the first few textures actually take. */
	{
		static int imp_prints = 0;
		if (imp_prints < 6) {
			imp_prints++;
			printf("IMP fmt=%d siz=%d line=%u bytes=%u setw=%u\n",
			       (int) fmt, (int) siz,
			       (unsigned) rdp.texture_tile.line_size_bytes,
			       (unsigned) rdp.loaded_texture[dslot].size_bytes,
			       (unsigned) last_set_texture_image_width);
		}
	}
#endif
...
#if defined(TARGET_XBOX)
/* Palette currently converted into tlut[]. See gfx_dp_load_tlut: repeated loads
 * of the same address within one frame are skipped, and this is cleared every
 * frame because the game animates palettes in place. */
static const void *gfx_tlut_last_src = NULL;
void gfx_tlut_frame_reset(void) { gfx_tlut_last_src = NULL; }
#endif

/* Pushes skipped because the modelview stack was full; their matching pops
 * must be cancelled, not applied (see gfx_sp_matrix / gfx_sp_pop_matrix). */
static uint32_t mv_skipped_pushes = 0;
#if MK64X_DEBUG_TOOLS
static int mv_stack_report = 0;
#endif

#if defined(TARGET_XBOX)
/* Last known-good MP_matrix, and a guard that refuses to install a broken one.
 *
 * Every vertex is transformed by MP_matrix, so a single non-finite element
 * turns a whole object's vertices into NaN. The backend then drops those
 * batches (GFXNAN) and the object VANISHES for that frame -- seen as the award
 * ceremony's floor flickering on and off, and it is the same failure the
 * armature limb matrices used to cause before guMtxF2L fixed those.
 *
 * The game itself cannot be the source: its matrices arrive as s16.16
 * fixed-point, which has no encoding for NaN or infinity. So a bad MP_matrix
 * is always produced HERE, by the multiply chain -- an accumulated overflow,
 * or a stack slot that was never written. Keeping the previous matrix for the
 * offending draw is strictly better than emitting geometry that gets thrown
 * away: the object stays where it was for one frame instead of disappearing. */
static float mp_last_good[4][4];
static int   mp_have_good = 0;
#if MK64X_DEBUG_TOOLS
static int   mp_reject_report = 0;
#endif

static void gfx_mp_matrix_guard(void) {
	const float *m = (const float *) rsp.MP_matrix;
	for (int i = 0; i < 16; i++) {
		/* NaN and +/-inf both fail (v - v == 0). */
		if (!(m[i] - m[i] == 0.0f)) {
#if MK64X_DEBUG_TOOLS
			/* Report only. The substitution below is a fix, not a probe, and
			 * stays compiled in either way. */
			if (mp_reject_report < 8) {
				mp_reject_report++;
				printf("MTXBAD non-finite MP_matrix rejected (elem %d)%s\n",
				       i, mp_have_good ? ", kept previous" : ", NO previous yet");
			}
#endif
			if (mp_have_good) {
				shz_matrix_4x4_copy(rsp.MP_matrix, mp_last_good);
			}
			return;
		}
	}
	shz_matrix_4x4_copy(mp_last_good, rsp.MP_matrix);
	mp_have_good = 1;
}
#else
#define gfx_mp_matrix_guard() ((void) 0)
#endif
...
static __attribute__((noinline)) void gfx_sp_matrix(uint8_t parameters, const int32_t* addr) {
	...
	if (parameters & G_MTX_PROJECTION) {
		...
		if (parameters & G_MTX_PUSH) {
			if (rsp.modelview_matrix_stack_size < 32) {
				...
				/* Stack full: the push must still be CANCELLED by its later
				 * pop, or balanced push/pop pairs at the cap drain the stack
				 * below where they started (the seagull explosion). Count the
				 * skip; gfx_sp_pop_matrix consumes it before really popping. */
				++mv_skipped_pushes;
#if MK64X_DEBUG_TOOLS
				if (mv_stack_report < 4) {
					mv_stack_report++;
					printf("MTXSTK push skipped (depth 32)\n");
				}
#endif
			}
		}
...
static void  __attribute__((noinline)) gfx_sp_pop_matrix(uint32_t count) {
	while (count--) {
		if (mv_skipped_pushes > 0) {
			/* This pop pairs with a push that was skipped at the cap. */
			--mv_skipped_pushes;
			continue;
		}
		/* Floor at 1, never 0: every reader (gfx_sp_matrix, the MP
		 * recompute, calculate_normal_dir) dereferences stack[size - 1],
		 * and size 0 made that stack[-1] -- out-of-bounds struct memory
		 * whose garbage floats were the NaN vertex source. */
		if (rsp.modelview_matrix_stack_size > 1) {
			--rsp.modelview_matrix_stack_size;
		}
#if MK64X_DEBUG_TOOLS
		else if (mv_stack_report < 4) {
			mv_stack_report++;
			printf("MTXSTK pop floored at 1\n");
		}
#endif
	}
...
static void __attribute__((noinline)) gfx_sp_vertex_light(size_t n_vertices, size_t dest_index, const Vtx* vertices) {
    ...
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        ...
        if (rsp.geometry_mode & G_TEXTURE_GEN) {
            ...
            float doty; // = 0;
            {
                SHZ_DOTREG(fr8)  = vn->n[0];
                SHZ_DOTREG(fr9)  = vn->n[1];
                SHZ_DOTREG(fr10) = vn->n[2];
                SHZ_DOTREG(fr11) = 0;
        ...
        cr = (cr << 1) | !(x < -w);
#if defined(TARGET_XBOX)
        // See the unlit loader: NaN/inf from a stale teardown matrix defeats
        // every clip comparison and later hangs the NV2A. Reject at the source.
...
static void __attribute__((noinline)) gfx_sp_vertex_no(size_t n_vertices, size_t dest_index, const Vtx* vertices) {
    ...
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
		...
		*((shz_vec4_t *)&d->_x) = shz_xmtrx_trans_vec4(*((shz_vec4_t *)&d->x));
		...
		cr = (cr << 1) | !(d->_x < -d->_w);
#if defined(TARGET_XBOX)
		// NaN/inf coordinates (a stale or zeroed matrix during course teardown)
		// defeat every clip test here BY COMPARING FALSE: each !(cmp) reads as
		// "inside", the w<=eps scissor test reads as "in front", and the vertex
		// sails through as fully visible -- then projects to NaN screen coords,
		// which hang the real NV2A's setup engine (the end-of-race hardlock).
		// The N64's fixed-point RSP could never produce these. Mark the vertex
		// outside every plane so any triangle touching it is rejected outright.
		// (x - x) == 0 only holds for finite values.
		if (!((d->_x - d->_x == 0.0f) && (d->_y - d->_y == 0.0f) &&
		      (d->_z - d->_z == 0.0f) && (d->_w - d->_w == 0.0f))) {
			cr = 0;
			d->wlt0 = 1;
			d->_x = d->_y = d->_z = 0.0f; d->_w = 1.0f;
		}
#endif
		d->clip_rej = cr;
...
static void  __attribute__((noinline)) GFX_HOT gfx_sp_tri1_impl(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx) {
    ...
    for (int ti = 0; ti < n_tris; ti++) {
        ...
        /* TRIED AND REVERTED: collapsing any triangle with a vertex at
         * _w <= SCISSOR_W_EPS to zero area. It removed the giant garbage
         * triangles, but it also ate the ceremony floor -- the floor is a
         * large quad under a low camera, and its own triangles legitimately
         * reach here with _w at the epsilon, so the flicker came straight
         * back. Whatever separates "clipped near-plane geometry" from
         * "garbage vertex sitting on the camera", _w alone is not it. */
		...
		for (i = 0; i < 3; i++) {
			...
			/* The comment above says _w > eps is guaranteed by the eye-plane
			 * clip. Trust it for the common case but do not BET on it: this
			 * reciprocal is the one place in the pipeline where a finite input
			 * becomes a non-finite output, and everything downstream inherits
			 * it. _w == 0 gives invw == inf, and then _x * invw is 0 * inf ==
			 * NaN for any vertex on the axis; a denormal _w overflows invw past
			 * float range to inf outright. The backend then drops the whole
			 * batch (GFXNAN) and the object vanishes for that frame -- the
			 * award ceremony's floor flickering, which survived both the vertex
			 * loaders' NaN rejection and the MP_matrix guard because it is born
			 * HERE, downstream of both.
			 *
			 * Clamping to the eye-plane epsilon puts such a vertex at the near
			 * plane, which is where the clip should have placed it anyway. The
			 * comparison is written so NaN takes the clamp too (NaN > x is
			 * false). */
			float vw_ = v_arr[i]->_w;
			if (!(vw_ > SCISSOR_W_EPS)) vw_ = SCISSOR_W_EPS;
			float invw = shz_inverse_posf(vw_);
			bv->vert.x = sm_xscale * (v_arr[i]->_x * invw) + sm_xbias;
			bv->vert.y = sm_yscale * (v_arr[i]->_y * invw) + sm_ybias;
#if defined(TARGET_XBOX)
			// PowerVR derives perspective correction from z (which IS 1/w there).
			// D3D8 needs rhw as its own component: without it texture coordinates
			// interpolate affinely and everything warps PS1-style.
			bv->rhw = invw;
#endif
...
#if defined(TARGET_XBOX)
			// PowerVR's depth is unbounded 1/w. D3D8 clips pre-transformed
			// vertices against z in [0,1], and this expression can exceed 1 --
			// near geometry (invw > 1) and the 2D pane scheme (screen_2d_z runs
			// 1.0 upward). Scaling by a constant keeps every relative ordering
			// intact, including the +0.0005 per-rect 2D increments, while
			// bringing the whole range inside the clip volume. Ordering is all
			// that matters: the comparison is GREATEREQUAL against a
			// zero-cleared buffer, so larger still means nearer.
			bv->vert.z *= XBOX_Z_SCALE;
#endif
#if GFX_PROF
...
#if defined(TARGET_XBOX)
				// D3D8 takes the fog FACTOR from specular alpha when the fog
				// table mode is NONE, and its convention is the INVERSE of the
				// N64/PVR one: 255 means no fog, 0 means fully fogged. Passing
				// the N64 coefficient straight through washes near geometry to
				// the fog colour while leaving the sky (drawn unfogged) alone.
				_oargb = (_oargb & 0x00FFFFFFu) |
				         ((uint32_t)(255u - v_arr[i]->fog) << 24);
#else
...
#if defined(TARGET_XBOX)
/* Set by gfx_draw_rectangle for a far-pinned BACKDROP only (a textured 2D quad
 * drawn before any 3D in a frame that has a 3D scene). Consumed by
 * gfx_sp_quad_2d. Deliberately narrower than opaque_2d -- see the note there. */
static uint8_t xb_far_backdrop = 0;
#endif
...
static void  __attribute__((noinline)) gfx_sp_quad_2d(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx, uint8_t vtx1_idx2, uint8_t vtx2_idx2,
	...
	uint8_t depth_test = (rsp.geometry_mode & G_ZBUFFER) == G_ZBUFFER;
#if defined(TARGET_XBOX)
	// Far-pinned 2D backdrops are handled by the backend's draw-first backdrop
	// pass (gfx_nv2a xb_is_backdrop): a Z-off batch at far-pinned z is flushed
	// before OP, restoring the painter order. Forcing depth TEST here instead
	// was tried and rejected the backdrop wherever anything wrote depth into
	// the sky region. Keep the flag consumed so it can't leak to the next quad.
	xb_far_backdrop = 0;
	(void) xb_far_backdrop;
#endif
...
#if defined(TARGET_XBOX)
		// Carry rhw across with the position: the source verts come from
		// gfx_draw_rectangle, which already set it. Dropping it here would
		// leave these quads with rhw == 0 and they would not rasterise.
		quad_vbo[tri_num_vert].rhw = v_arr[tri_num_vert]->rhw;
#endif
...
#if defined(TARGET_XBOX)
			// The 2D path computes no per-vertex fog, and never touches the
			// backend's fog enable either -- so it inherits whatever the 3D
			// path last set. Specular alpha 0 reads as "no fog" on PVR but as
			// "fully fogged" in D3D8, which would flood these quads with the
			// fog colour. 0xFF is D3D8's "no fog".
			quad_vbo[tri_num_vert].pad0.vertindex = _oargb | 0xFF000000u;
#else
...
#if defined(TARGET_XBOX)
		// 2D screen-space quads rely on PAINTER order: the menus draw
		// background strips, then the selection box (draw_flash_select_case /
		// draw_box_fill in menu_items.c), then the option text -- all
		// sequentially. Splitting them across OP/PT/TR buckets breaks that:
		// the opaque selection boxes classified OP flushed FIRST and the ~150
		// TR background strips painted straight over them, which is why the
		// selected menu item was invisible. Route ALL 2D quads to TR: their
		// ascending screen_2d_z encodes emission order and the TR flush sorts
		// ascending, so painter order is preserved exactly; opaque quads stay
		// opaque under blending (vertex alpha 255).
		q_kind = 2;
#endif
		gfx_pvr_set_blend(q_kind);
		if (q_kind == 2) {   // TR: derive + push blend factors from the N64 blender
			uint8_t bs, bd;
			// (An earlier ONE/ZERO force for ex-OP quads lived here to patch
			// over item icons whose bodies APPEARED to use a=0 palette
			// entries -- an artifact of the convention-scrambled palettes.
			// With ROM-true palettes every body texel is a=1 and derived
			// factors are correct for all 2D quads; the force instead painted
			// the a=0 backgrounds of portraits/icons as garbage colour.)
			pvr_derive_blend(rdp.other_mode_l, rdp.other_mode_h, &bs, &bd);
			gfx_pvr_set_blend_factors(bs, bd);
			pvr_cur_bsrc = bs; pvr_cur_bdst = bd;
		}
...
static void  gfx_dp_set_tile(uint32_t w0, uint32_t w1) {
	...
	// shiftt (C1(10,4)) and shifts (C1(0,4)) are not consumed by this port.
	if (tile == G_TX_RENDERTILE) {
		// palette selects the CI4 sub-palette (upper 4 bits of the colour
		// index); needed by import_texture_ci4.
		rdp.texture_tile.palette = (uint8_t) C1alt(20, 4);
...
extern uint16_t common_tlut_hud_type_C_rank_tiny_font[];
...
static void  __attribute__((noinline)) gfx_dp_load_tlut(UNUSED uint32_t high_index) {
...
#if defined(TARGET_XBOX)
	/* A NULL palette address means the caller drew an object whose palette was
	 * never assigned (Banshee's bats in Time Trials: update_bat, which sets
	 * activeTLUT, is skipped for that mode while render_object_bat ran anyway).
	 * N64 reads address 0 as ordinary RDRAM and the DC runs with the MMU off,
	 * so both just loaded garbage; the Xbox access-violates. Keep the previous
	 * palette and skip -- a wrong palette beats a hard crash, and the print
	 * names any other caller with the same defect. */
	if (rdp.texture_to_load.addr == NULL) {
		static int sNullTlut = 0;
		if (sNullTlut < 4) {
			sNullTlut++;
			printf("TLUTNULL load_tlut with NULL palette (course=%d) -- skipped\n",
...
#if defined(TARGET_XBOX)
	/* Skip a redundant reload of the palette already sitting in tlut[].
	 *
	 * Display lists reload the same palette before nearly every draw that uses
	 * it: the award ceremony issues ~100 gDPLoadTLUT_pal256 calls per frame for
	 * ONE address (its confetti and karts all share a palette), and each one
	 * re-converts 256 entries. Converting a palette that is already converted
	 * is pure waste.
	 *
	 * Only a repeat of the IMMEDIATELY PRECEDING address is skipped, because
	 * tlut[] holds exactly one palette -- an A,B,A sequence still reloads. And
	 * the cache is dropped every frame (gfx_tlut_frame_reset), because the game
	 * rewrites palette CONTENTS in place at the same address between frames:
	 * that is how the kart wheels spin, and treating those as unchanged would
	 * freeze the animation. Within a single frame the game builds its display
	 * list before any of it is walked, so contents cannot change mid-walk. */
	if (srcp == gfx_tlut_last_src) {
		return;
	}
	gfx_tlut_last_src = srcp;
#endif

#if defined(TARGET_XBOX) && MK64X_DEBUG_TOOLS
	/* WHITE-gated: which palette source does each draw REALLY use (compiled
	 * C array in the exe image vs. the loaded common_data.bin segment), and
	 * in which byte order does it arrive? seg= the pre-translation address
	 * (0x0Dxxxxxx = segment), v= the raw first u16s. */
	{
		extern int printf(const char*, ...);
		if (xbox_gfx_log_once) {
			const uint16_t* r16 = (const uint16_t*) srcp;
			printf("TLUT seg=%08X v=%p raw=%04X %04X %04X %04X\n",
			       (unsigned) (uintptr_t) rdp.texture_to_load.addr, (void*) srcp,
			       r16[1], r16[2], r16[3], r16[4]);
		}
	}
#endif
	uint16_t clearcolor;
	/* These two rank-font palettes are the only 16-bit common_data arrays
	 * drawn by SYMBOL that main.c does not byteswap at boot (see
	 * tools/audit_symbol_swaps.py), so their alpha-0 entries still arrive in
	 * value order and have to be swapped here. The tiny variant was missing
	 * from this test even though it is loaded exactly the same way. */
	int hud_thing = (srcp == segmented_to_virtual(common_tlut_hud_type_C_rank_font)) ||
	                (srcp == segmented_to_virtual(common_tlut_hud_type_C_rank_tiny_font));
	...
	/* Input is ALWAYS a raw BE byte stream. (A per-palette orientation
	 * heuristic briefly lived here and made the kart flash during wheel-spin
	 * palette animation -- the real fix was making the SEGMENT copy of
	 * common_data ROM-true, since the shell draw DLs reload their palette
	 * from the segment, not from the boot-swapped C arrays.) */
	...
	for (int i = 0; i < 128; i++) {
		...
		if (a || hud_thing) {
			...
			c1 = clearcolor;   /* finish-line banner: a=0 forced opaque white */
		} else {
			/* KEEP the colour, alpha stays 0. Substituting transparent BLACK
			 * here destroyed the RGB of every a=0 palette entry -- and the
			 * shell bodies are BUILT from those (823/1280 icon pixels, 537 of
			 * them interior): on N64 they draw solid because those paths
			 * ignore texel alpha. Black/holey shells came from this line. */
			c1 = __builtin_bswap16(c1);
			c1 = (((c1 >> 11) & 0x1f) << 10) | (((c1 >> 6) & 0x1f) << 5) | ((c1 >> 1) & 0x1f);
		}
		a = !!(c2 & 0x100);
		if (a || hud_thing) {
			...
			c2 = __builtin_bswap16(c2);
			c2 = (((c2 >> 11) & 0x1f) << 10) | (((c2 >> 6) & 0x1f) << 5) | ((c2 >> 1) & 0x1f);
		}
		*tlp32++ = (c2 << 16) | c1;
	}
}
...
static void  __attribute__((noinline)) gfx_draw_rectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry) {
	...
	if (prev_frame_had_persp && !has_done_3d && !do_fill_rect) {
		rz = 0.00001f;
#if defined(TARGET_XBOX)
		xb_far_backdrop = 1;   /* consumed by gfx_sp_quad_2d */
#endif
	}
...
#if defined(TARGET_XBOX)
	// Same clip-range fold as the 3D path (see gfx_sp_tri1): screen_2d_z climbs
	// past 1.0 across a frame's rects, which D3D8 would clip away. Applied once
	// here rather than per corner.
	rz *= XBOX_Z_SCALE;
#endif
	...
	ur->vert.z = rz;
#if defined(TARGET_XBOX)
	// 2D quads are already in screen space with w == 1, so rhw is unity. The
	// 3D path sets this from invw; these four never pass through it.
	ul->rhw = ll->rhw = lr->rhw = ur->rhw = 1.0f;
#endif
